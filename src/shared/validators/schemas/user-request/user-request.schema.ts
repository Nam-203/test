...
export const BaseUserRequestSchema = z.object({
	...
	typeId: z.union([
		z.string().min(1, "Request type is required."),
		z.nativeEnum(EUserRequestType),
	]),
...
export const CreateHolidayRequestSchema = BaseUserRequestSchema.pick({
	...
	companyIds: z
		.array(z.number())
		.nonempty({ message: "At least one company is required." }),
});
...
export const EditUserRequestSchema = BaseUserRequestSchema.omit({
	deletedAt: true,
	createdAt: true,
	updatedAt: true,
});
...
export type TCreateHolidayRequestReq = z.infer<
