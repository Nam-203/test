...
export const isUnprocessableEntityError = <T>(
	error: unknown,
): error is AxiosError<T> => {
	return (
		(error as AxiosError<T>).isAxiosError &&
		(error as AxiosError<T>).response?.status ===
...
export function isExpiredAccessTokenError<UnauthorizedError>(
	error: unknown,
): error is AxiosError<UnauthorizedError> {
	return (
...
export function isInValidAccessTokenError<UnauthorizedError>(
	error: unknown,
): error is AxiosError<UnauthorizedError> {
	return (
		isUnauthorizedError<TApiError<typeof UnauthorizedCauseSchema>>(error) &&
...
export function isInValidRefreshTokenError<UnauthorizedError>(
	error: unknown,
): error is AxiosError<UnauthorizedError> {
	return (
		isUnauthorizedError<TApiError<typeof UnauthorizedCauseSchema>>(error) &&
		error.response?.data.data?.cause ===
			UNAUTHORIZED_CAUSE.REFRESH_TOKEN.INVALID
	);
}

export function isExpiredRefreshTokenError<UnauthorizedError>(
	error: unknown,
): error is AxiosError<UnauthorizedError> {
	return (
		isUnauthorizedError<TApiError<typeof UnauthorizedCauseSchema>>(error) &&
		error.response?.data.data?.cause ===
			UNAUTHORIZED_CAUSE.REFRESH_TOKEN.EXPIRED
	);
}
