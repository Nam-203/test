...
export const formatDateFromISO = (isoString: string): string => {
	...
	// Format the date string with the specified time zone
	return format(date, "yyyy-MM-dd HH:mm:ss");
};

export const compareTime = (
	start: string | TTimeObject,
	end: string | TTimeObject,
): boolean => {
	const startDate =
		typeof start === "string"
	...
	const endDate =
		typeof end === "string"
			...
			: setMinutes(setHours(new Date(), end.hour), end.minute);
...
export const formatObjTimeToTimeString = (timeObj: {
	...
	return format(
		setMinutes(setHours(new Date(), timeObj.hour), timeObj.minute),
		"HH:mm",
	);
};
 ...
 */
export const parseTimeStringToObjTime = (
	timeStr: string | TTimeObject | null,
): TTimeObject | null => {
...
export const convertTimeObjectToDate = (
	time: TTimeObject | null,
): Date | null => {
	if (!time) return null;
	return setMinutes(setHours(new Date(), time.hour), time.minute);
};

export function formatDayNameByLanguage(
	dayNumber: number,
	language: string,
): string {
...
export function formatDateTimeByLanguage(
	...
	switch (language) {
		...
		default:
			return format(
				new Date(isoString),
				"EEEE, 'ngày' dd 'tháng' M, yyyy 'lúc' HH:mm",
				{
					locale: vi,
				},
			);
