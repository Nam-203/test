...
const ResendVerifyAccountForm = () => {
	...
	const { mutate, isPending } = useMutation({
		mutationFn: (email: string) =>
			authApiRequest.resendVerifyAccount({ email }),
		onSuccess: (data) => {
			customToast.success({
			...
			<div className="flex w-[90%] max-w-[500px] flex-col justify-center rounded-lg bg-white p-8 text-center shadow-s-light-b-strong">
				<div className="mb-4 flex justify-center">
					<Image
						...
						alt="resend-verify-account-logo"
						width={250}
						height={250}
					/>
				</div>
				<div className="mb-8 text-xl font-semibold text-primary">
					{t("auth:resendVerifyAccount.title")}
				</div>
...
export default ResendVerifyAccountForm;
